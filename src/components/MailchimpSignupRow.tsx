  ...
  const ghostButton =
    variant === "ghost"
      ? "relative border-2 border-white/25 bg-white/[0.03] backdrop-blur-md text-white/90 " +
        ...
        "hover:bg-bawo-eggplant/30 hover:border-bawo-peach/45 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_6px_20px_rgba(0,0,0,0.3),0_0_32px_rgba(255,107,0,0.1)] " +
    ...
    >
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 sm:items-end">
        <input
          ...
          className={`flex-1 min-w-0 bg-transparent border-0 border-b-2 border-white/25 rounded-none shadow-none focus:ring-0 focus:outline-none focus:border-bawo-peach transition-colors duration-200 font-museo-medium text-white placeholder:text-bawo-peach placeholder:opacity-90 ${inputTypography}`}
        />
        <button
          type="submit"
          disabled={status === "loading"}
